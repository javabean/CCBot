...
public class HttpRequestExecutor extends HttpAccessor {
		...
		if (extraParameters != null && !extraParameters.isEmpty()) {
			switch (method) {
				...
				// add form parameters in request body
					...
					PrintStream body = new PrintStream(httpRequest.getBody(), true, StandardCharsets.UTF_8.name());
		...
		for (Map.Entry<String, ?> entry : params.entrySet()) {
			if (entry.getValue() != null) {
				...
				try {
					...
					.append('=')
					.append(URLEncoder.encode((entry.getValue() == null ? "" : entry.getValue().toString()), StandardCharsets.UTF_8.name()))
					.append('&');
	...
	private static final Charset DEFAULT_HTTP_ENCODING = StandardCharsets.ISO_8859_1;
	/**
	 * @param httpMessage
